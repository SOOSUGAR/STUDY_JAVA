...
public class CalendarTest01 {
	public static void main(String[] args) {
		// 1. Calendar 클래스의 인스턴스 생성  - 시스템의 날짜를 가져와서 사용
		// - 추상 클래스이므로 new가 아니라, getInstance() 메소드를 통해서 인스턴스 생성
		Calendar c = Calendar.getInstance();
		
		int year = c.get(Calendar.YEAR);
		int month = c.get(Calendar.MONTH) + 1; // 0 ~ 11까지의 값, 1을 더하여 보정
		...
		System.out.println("────── Calendar 클래스에 추가된 기능 ──────");
		...
		System.out.println("올해의 몇 번째 일 : " + c.get(Calendar.DAY_OF_YEAR));
		// DAY_OF_WEEK - 1:일요일, 2:월요일, 3:화요일, 4:수요일, 5:목요일, 6:금요일, 7:토요일 
		System.out.println("이주의 몇 번째 일 : " + c.get(Calendar.DAY_OF_WEEK));
		
		// 1. 요일을 만드는 방법
		String[] dayofweek = {"", "일", "월", "화", "수", "목", "금", "토"};
		System.out.println("오늘의 요일 : " + dayofweek[c.get(Calendar.DAY_OF_WEEK)]);

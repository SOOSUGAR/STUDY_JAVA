...
class Account {
	...
	// 입금 메소드 -> 음수를 입력하여 입금을 하게 되는 경우 발생
	/*
	// 1번 방법 - 논리적인 방법으로 해결
	...
	*/
	...
	public void deposit(int money) throws MinusDepositException {
		if(money <=0 ) {
			throw new MinusDepositException("입금 오류 : " + money + "원은 입금할 수 없습니다.");
		}
		balance += money;
		System.out.println(name + "님의 계좌에서 " + money + "원이 입금되었습니다.");
	}
	
	// 출금 메소드 -> 예금잔고보다 큰 금액을 인출하게 되는 경우 발생
	/*
	// 1번 방법 - 논리적인 방법으로 해결
	...
	*/
	...
	public void withdraw(int money) throws BalanceInsuffientException {
		if(balance < money) {
			throw new BalanceInsuffientException("출금 오류 : " + (money - balance) + "원이 부족합니다.");
		}
		balance -= money;
